defmodule AoC.Day11 do
  ...
  defp run_steps(grid, step_count) do
    1..step_count |> Enum.reduce_while({grid, 0}, &run_step/2)
  end

  defp run_step(step, {grid, total_flash_count}) do
    grid = grid |> Grid.get_points() |> Enum.reduce(grid, &increment(&1, &2, &2[&1]))

    flash_count = Grid.count(grid, nil)

    if flash_count == Grid.count(grid) do
      {:halt, step}
    else
      grid = Grid.replace(grid, nil, 0, :all)
  ...
  defp increment(octopus, grid, 9) do
    ...
    |> Enum.reduce(Grid.set(grid, octopus, nil), &increment(&1, &2, &2[&1]))
  end
